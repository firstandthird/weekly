...
(function($) {
  ...
  $.declare('weekly', {
    ...
    highlightToday: function() {
      ...
      var startTime = today.toTimeString().slice(0,5);
      var todayEl = this.el.find('.weekly-grid [data-date="' + TimeFormat('%Y-%n-%j', today) + '"]');

      todayEl.addClass('weekly-today');

      if(todayEl.length) {
        this.el.find('.weekly-grid .weekly-current-time').css({
          top: (100 - this.getTimeOffsetPercent(startTime)) + '%',
          display: 'block'
        });
      }
    },
