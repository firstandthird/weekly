...
(function($) {
  ...
  $.declare('weekly', {
    ...
    registerClickToCreate: function() {
      ...
      gridDays.on('mouseup', this.proxy(function(){
        ...
        if(this.pendingEvent) {
          ...
          if (!this.isWithinWhitelistedRange(dateUtils.timeToTwelve(startHours, startMins), dateUtils.timeToTwelve(endHours, endMins))) {
            this.pendingEvent.remove();
            this.pendingEvent = null;
            this.pendingEventStart = null;
            return;
    ...
    modifyEvent: function(event) {
      ...
      if (this.intersects(target).length || !this.isWithinWhitelistedRange(startTime, endTime)) {
        this.currentDragger.trigger('mouseup');
        target.css({
    ...
    canAdd: function(date, time) {
      ...
      for (var i = 0, length = this.canAdd_.length; i < length && canAdd; i++) {
        ...
        if ($.type(validator) === 'function') {
          validatorResult = validator(date, time);
    ...
    setSelectableDates: function(dates, skipUpdate) {
      if (dates){
        ...
        if ($.type(dates) === 'function'){
          this.canAdd_.push(dates);
        } else {
          this.canAdd_.push(function (date) {
            if (date) {
    ...
    generateBlacklistedTimes: function(times) {
      var whitelistedTimes = [];
      var i, length;
      var start = null;

      this.blackListedTimes = [];

      for (i = this.startTime; i < this.endTime + 12; i++) {
        var time = dateUtils.timeToTwelve(i);

        if (times.indexOf(time) > -1) {
          whitelistedTimes.push(time);
        } else {
          whitelistedTimes.push(null);
        }
      }

      for (i = 0, length = whitelistedTimes.length; i < length; i++) {
        var time2 = whitelistedTimes[i];

        if (time2) {
          if (start !== null) {
            this.blackListedTimes.push({
              start: start * 60,
              end: i * 60
            });
            start = null;
    ...
    setWhitelistedTimes: function(times, skipUpdate) {
      if (times){
        if ($.type(times) === 'function'){
          ...
          this.generateBlacklistedTimes(times);
    ...
    isWithinWhitelistedRange: function(start, end) {
      var whitelisted = true;

      if (this.whitelistedTimes.indexOf(dateUtils.absoluteTime(start)) === -1) {
        whitelisted = false;
      } else {
        // If time is exact, it may fall in the beginning of a not-whitelisted time
        if (end.split(':').pop().indexOf('00') > -1){
          whitelisted = this.whitelistedTimes.indexOf(dateUtils.changeHours(end, -1)) > -1;
        } else {
          if (this.whitelistedTimes.indexOf(dateUtils.absoluteTime(end)) === -1) {
            whitelisted = false;
          } else { // Most complex case, we could be covering an invalid time
            for (var i = 0, length = this.blackListedTimes.length; i < length && whitelisted; i++) {
              var blacklist = this.blackListedTimes[i];
              var startMins = dateUtils.timeToMinutes(start);
              var endMins = dateUtils.timeToMinutes(end);

              if (startMins <= blacklist.start && endMins >= blacklist.end) {
                whitelisted = false;
              }
            }
          }
