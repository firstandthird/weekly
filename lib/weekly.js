(function($) {
  ...
  $.declare('weekly', {
    ...
    modifyEvent: function(event) {
      ...
      var modEnd = new Date(end.getTime());
          modEnd.setMinutes(end.getMinutes() - this.interval);

      if(!this.allowOverlap && this.overlaps(start.getTime(), modEnd.getTime(), target.data('_index'))) {
        return;
      }
