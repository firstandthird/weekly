...
(function($) {
  ...
  $.declare('weekly', {
    ...
    update: function() {
      ...
      for (i = 0, c = this.events.length; i < c; i++) {
    ...
    registerClickToCreate: function() {
      ...
      gridDays.on('mouseup', this.proxy(function() {
        ...
        if (this.pendingEvent) {
          ...
          var startDate = moment(eventData.date).tz(this.timezone);
          var endDate = startDate.clone();

          startDate.hours(eventData.starttime);
          startDate.minutes(this.fromDecimal(eventData.starttime));
          endDate.hours(eventData.endtime);
          endDate.minutes(this.fromDecimal(eventData.endtime));
    ...
    initializeEventConfig: function(event, offsetTarget) {
      var target = $(event.currentTarget);
      offsetTarget = offsetTarget || target;
      var targetOffset = offsetTarget.parent().offset();
      var mouseOffsetTop = event.pageY - targetOffset.top;
      var dayHeight = target.height();
      ...
      return {
        ...
        mouseOffsetTop: mouseOffsetTop,
        hourHeight: hourHeight,
        intervalHeight: intervalHeight,
        minHeight: minHeight,
        dayHeight: dayHeight,
        temp: {
          start: Math.floor(mouseOffsetTop / intervalHeight) * intervalHeight,
          end: Math.ceil(mouseOffsetTop / intervalHeight) * intervalHeight
        }
      };
    },
    ...
    showPreviewEvent: function(event) {
      var previewEvent;
      var eventConfig = this.initializeEventConfig(event);

      eventConfig.temp.end = eventConfig.temp.start + eventConfig.minHeight;

      var eventDate = eventConfig.target.data('date');

      var startTime = ((eventConfig.temp.start / eventConfig.hourHeight) || 0) + this.startTime;
      var endTime = ((eventConfig.temp.end / eventConfig.hourHeight) || 1) + this.startTime;

      if (endTime - startTime < this.minDuration / 60) {
        endTime = startTime + (this.minDuration / 60);
      }

      var start = moment(eventDate).hours(startTime).minutes(this.fromDecimal(startTime));
      var end = moment(eventDate).hours(endTime).minutes(this.fromDecimal(endTime));

      eventConfig.target.append('<div class="weekly-event-preview">'
                      + '<div class="weekly-event-preview-time weekly-event-time">'
                        +  start.format('h:mm') + ' - ' + end.format('h:mmA')
                      + '</div>'
                    + '</div>');

      previewEvent = eventConfig.target.find('.weekly-event-preview');

      previewEvent.css({
        top: eventConfig.temp.start,
        bottom: eventConfig.dayHeight - eventConfig.temp.end
      });
    },
    ...
    createEvent: function(event) {
      ...
      if (eventConfig.temp.start === eventConfig.temp.end) {
        eventConfig.temp.end += eventConfig.intervalHeight;
      }

      if (this.pendingEventStart === null) {
        this.pendingEventStart = eventConfig.temp.start;
      }

      if (this.pendingEventEnd === null || eventConfig.temp.end > this.pendingEventStart) {
        this.pendingEventEnd = eventConfig.temp.end;
      }

      var startTime = ((this.pendingEventStart / eventConfig.hourHeight) || 0) + this.startTime;
      ...
      if (!this.allowOverlap && this.overlaps(start.toDate().getTime(), end.toDate().getTime())) {
      ...
      if (!this.pendingEvent) {
        eventConfig.target.append('<div class="weekly-event-pending"></div>');
        this.pendingEvent = eventConfig.target.find('.weekly-event-pending');
        this.pendingEvent.data('date', eventConfig.target.data('date'));
      }

      this.pendingEvent.css({
        top: this.pendingEventStart,
        bottom: eventConfig.dayHeight - this.pendingEventEnd
      });
    ...
    modifyEvent: function(event) {
      var eventConfig = this.initializeEventConfig(
        event,
        this.currentDragger.parents('.weekly-event')
      );
      var target = eventConfig.target;

      var duration = target.outerHeight() / eventConfig.hourHeight;
      var start = moment(target.data('start'));
      var end = moment(target.data('start'));

      end.add(~~(duration), 'hours');
      end.add(this.fromDecimal(duration), 'minutes');

      if (this.intersects(target).length) {
        this.currentDragger.trigger('mouseup');
        target.css({
          bottom: eventConfig.dayHeight - eventConfig.temp.end + eventConfig.intervalHeight
        });
        return;
      }

      eventConfig.target.data('end', end.toDate());

      if (eventConfig.temp.end < (eventConfig.targetOffset.top + eventConfig.dayHeight)) {
        target.css({
          bottom: eventConfig.dayHeight - eventConfig.temp.end
        });
      }

      this.events[target.data('eventIndex')].end = end.toDate();

      this.el.trigger('modifyEvent', this.events[target.data('eventIndex')]);
    },
    ...
    renderEvent: function(event) {
      var start = moment(event.start).tz(this.timezone);
      var end = moment(event.end).tz(this.timezone);
      ...
      eventTemplate.data('offset-start', start.toDate());
      eventTemplate.data('offset-end', end.toDate());
    ...
    getTimeOffsetPercent: function(time) {
      ...
      return (diff / this.timeDifference) * 100;
    },
    ...
    updateEventScroll: function(startDate) {
      ...
      this.scrollTimer = setTimeout(this.proxy(function() {
        if (this.scrollFirstEvent) {
          var top = $(window).scrollTop();
          var scrollDate = this.scrollFirstEvent;
          var el;
          ...
          if (el.length) {
            ...
            el.each(function(i, el) {
              if (!first || el.offsetTop < first.offsetTop) {
                first = el;
              }
            });
